      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm"  class="picture-ruleForm" label-width="100px">
        ...
        <el-form-item label="上级分类" prop="" v-if="inputShow !== 1">
          <div class="superior">
            ...
            <el-select  v-show="inputShow >= 4" class="select-input" disabled v-model="ruleForm.select_4" placeholder="请选择三级菜单">
              <el-option v-for="(item,index) in options3" :key="index" :label="item.pname" :value="item.cid">
              </el-option>
            </el-select>
            <el-select  v-show="inputShow >= 5" class="select-input" disabled v-model="ruleForm.select_5" placeholder="请选择四级菜单">
