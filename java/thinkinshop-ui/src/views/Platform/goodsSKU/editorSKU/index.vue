  ...
  <div class="container">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item class="attribute-values" label="属性值" prop="attributeValues">
        <el-input v-model="ruleForm.attributeValues" placeholder="请输入属性值"></el-input>
        <div class="add-reduction">
          <i class="el-icon-remove-outline" @click="minus" v-if="attributeList.length !== 0"></i>
          <i class="el-icon-circle-plus-outline" @click="addOne" v-if="attributeList.length === 0"></i>
        </div>
      </el-form-item>
      <el-form-item v-if="attributeList.length !== 0" class="add-attribute" label="" prop="code">
...
<script>
import { getSkuAttributeList, addSku } from '@/api/Platform/goodsSku'
export default {
  name: 'editorSKU',
        ...
        if (valid) {
          try {
            addSku({
              api: 'saas.dic.addSku',
              sid: this.sid,
              attributeList: this.attribute ? this.attribute + ',' + this.ruleForm.attributeValues : this.ruleForm.attributeValues,
              type: 0
            }).then(res => {
              console.log(res);
